 ...
 */
#define S_FUNCTION_NAME                sfunar_fileWrite
...
#define PREALLOC        (ssGetSFcnParam(S, 1))
#define RINGBUF_SIZE    (ssGetSFcnParam(S, 2))
...
static void mdlRTW(SimStruct *S)
{
    ...
    uint32_T prealloc = (uint32_T)mxGetPr(PREALLOC)[0];
    uint32_T ringbufsize = (uint32_T)mxGetPr(RINGBUF_SIZE)[0];

    if (!ssWriteRTWParamSettings(S, 3,
                    SSWRITE_VALUE_DTYPE_NUM,    "Prealloc",      &prealloc,    DTINFO(SS_UINT32, COMPLEX_NO),
                    SSWRITE_VALUE_DTYPE_NUM,    "RingbufSize",   &ringbufsize, DTINFO(SS_UINT32, COMPLEX_NO),
                    SSWRITE_VALUE_STR,          "Filename",      mxArrayToString(FILENAME)))
    {
