...
/*
 *   sfunar_WireRead.c Simple C-MEX S-function for function call.
 *
 */
 ...
 */
#define S_FUNCTION_NAME                sfunar_WireRead
...
static void mdlSetWorkWidths(SimStruct *S)
{
   ...
   */
  ssRegDlgParamAsRunTimeParam(S, 2, 0, "p1", ssGetDataTypeId(S, "uint8"));
...
{
    uint32_T init[32], size;
    int32_T i, n, master;

    master = (int32_T)mxGetPr(MASTER)[0];

    n = (uint32_T)mxGetNumberOfElements(COMMAND);
    if(n > (sizeof(init)/sizeof(init[0]))){
        ssSetErrorStatus(S,"Maximum size of slave init code reached.");
    }

    for(i=0;i<n;i++){
        init[i] = (uint32_T)mxGetPr(COMMAND)[i];
    }

    size = (uint32_T)mxGetPr(SIZE)[0];
    if (!ssWriteRTWParamSettings(S, 4,
                    SSWRITE_VALUE_DTYPE_NUM,    "Master",    &master,   DTINFO(SS_UINT32, COMPLEX_NO),
                    SSWRITE_VALUE_DTYPE_VECT,   "Command",   init, n,   DTINFO(SS_UINT32, COMPLEX_NO),
                    SSWRITE_VALUE_DTYPE_NUM,    "Size",      &size,     DTINFO(SS_UINT32, COMPLEX_NO),
